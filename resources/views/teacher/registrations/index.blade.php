<x-app-layout>
    <div class="flex flex-col md:flex-row justify-between md:items-end gap-4 mb-6">
        <div class="flex items-center gap-3">
            <div class="w-12 h-12 bg-[#F59E0B]/10 text-[#F59E0B] rounded-xl flex items-center justify-center">
                <span class="material-symbols-outlined text-[28px]">how_to_reg</span>
            </div>
            <div>
                <h3 class="font-headline-md text-headline-md text-on-surface font-bold">Pendaftaran Siswa</h3>
                <p class="text-body-md text-secondary">Daftar siswa yang menunggu persetujuan Anda</p>
            </div>
        </div>
        
        <form action="{{ route('teacher.registrations.index') }}" method="GET" class="flex items-center gap-2">
    ...
    <div class="bg-white rounded-xl border border-outline-variant card-shadow overflow-hidden">
        <div class="p-4 bg-surface-container-low border-b border-outline-variant flex justify-between items-center">
            <h4 class="font-title-md font-bold text-on-surface flex items-center gap-2">
                <span class="material-symbols-outlined text-primary">pending_actions</span> Menunggu Persetujuan
            </h4>
        </div>
    ...
    <div id="detailModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/60 backdrop-blur-sm opacity-0 transition-opacity duration-300">
        <div class="bg-white rounded-xl w-[320px] overflow-hidden shadow-xl transform scale-95 transition-transform duration-300 relative border border-outline-variant/30">
            ...
            <div class="p-5 pb-3 flex flex-col items-center border-b border-outline-variant/50 bg-surface-container-lowest">
                ...
                <div id="modalStatus" class="mt-2 px-3 py-1 rounded-full text-[10px] font-bold bg-[#F59E0B]/10 text-[#F59E0B]">PENDING</div>
            </div>
    ...
    <script>
        function openDetailModal(id, name, className, email, phone, gender, ekskul, status, absen) {
            ...
            if(status === 'rejected') {
                ...
                const baseUrl = "{{ route('teacher.registrations.update', ':id') }}";
