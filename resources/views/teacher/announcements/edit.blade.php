<x-app-layout>
    <div class="pt-24 px-8 pb-12">
        <div class="mb-8">
            ...
            <h3 class="font-headline-md text-headline-md font-bold text-on-surface">Edit Pengumuman</h3>
            <p class="text-body-md text-secondary mt-1">Perbarui informasi pengumuman Anda.</p>
        </div>

        <div class="bg-white rounded-xl border border-outline-variant card-shadow overflow-hidden max-w-3xl">
            <form action="{{ route('teacher.announcements.update', $announcement->id) }}" method="POST" class="p-8 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select name="extracurricular_id" class="w-full px-4 py-3 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 bg-surface-container-lowest font-body-md" required>
                        ...
                        @foreach($ekskuls as $ek)
                            <option value="{{ $ek->id }}" {{ (old('extracurricular_id', $announcement->extracurricular_id) == $ek->id) ? 'selected' : '' }}>{{ $ek->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title', $announcement->title) }}" placeholder="Contoh: Perubahan Jadwal Latihan Minggu Ini" class="w-full px-4 py-3 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 bg-surface-container-lowest font-body-md" required>
                    @error('title')
                        <p class="text-error text-body-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="content" rows="6" placeholder="Tulis pesan pengumuman Anda di sini..." class="w-full px-4 py-3 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary/20 bg-surface-container-lowest font-body-md" required>{{ old('content', $announcement->content) }}</textarea>
                    @error('content')
                        <p class="text-error text-body-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 mt-2">
                        <label class="flex items-center p-4 border border-outline-variant rounded-lg cursor-pointer hover:bg-surface-container-low transition-colors flex-1">
                            <input type="radio" name="status" value="published" class="h-4 w-4 text-primary focus:ring-primary border-outline" {{ old('status', $announcement->status) == 'published' ? 'checked' : '' }}>
                            <div class="ml-3">
                        ...
                        <label class="flex items-center p-4 border border-outline-variant rounded-lg cursor-pointer hover:bg-surface-container-low transition-colors flex-1">
                            <input type="radio" name="status" value="draft" class="h-4 w-4 text-primary focus:ring-primary border-outline" {{ old('status', $announcement->status) == 'draft' ? 'checked' : '' }}>
                            <div class="ml-3">
